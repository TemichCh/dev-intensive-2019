...
class Bender(var status: Status = Status.NORMAL, var question: Question = Question.NAME) {
    ...
    fun listenAnswer(answer: String): Pair<String, Triple<Int, Int, Int>> {
        return when  {
            //(answer.isNullOrEmpty())->"${question.question}" to status.color
            (question == Question.NAME &&(answer.isNotEmpty() && answer[0].isLowerCase())) -> "Имя должно начинаться с заглавной буквы\n${question.question}" to status.color
            (question == Question.PROFESSION && (answer.isNotEmpty() && answer[0].isUpperCase())) -> "Профессия должна начинаться со строчной буквы\n${question.question}" to status.color
